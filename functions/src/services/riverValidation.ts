 ...
 * Frontend Mirror: src/utils/riverValidation.ts
 */
...
export function validateRiver(river: any): RiverValidationResult {
  ...
  try {
    ...
    // Buffer length fallback since Node (Functions) can use Buffer.from
    const byteLength = Buffer.from(jsonStr).length;
    if (byteLength > 25000) {
