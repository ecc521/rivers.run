...
function replaceStateNamesWithCodes(sentence, options = {}) {
  for (let stateCode in stateCodeToName) {
    if (stateCode === "FLA") {
      continue
    } //Officially FL. See above. Also, FLA contains FL.
    ...
    if (options.onlyAtEnd) {
      expressionText += "$"
    } //State Name must be at end of sentence (don't match "North Fork Mississippi River")
    if (options.notStart) {
      expressionText = "(?!^)" + expressionText
    } //State name may not be at the start (don't match "Mississippi").
